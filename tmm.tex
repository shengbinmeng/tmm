...
\begin{abstract}
In order to give the user best quality of service under bandwidth fluctuation, video streaming systems should be able to adjust the video quality according to available bandwidth. In this paper we propose such an adaptive video streaming system which adopts the Scalable Video Coding (SVC) technology and integrates novel models and algorithms. First, we utilize the Proportional-Integral-Derivative (PID) method to control the video quality adjustment according to actual bandwidth condition. By considering bandwidth information of the past, the current and even the future, the PID-based quality control algorithm is able to determine a suitable bitrate for transmission and deliver highest video quality with least fluctuation. Second, we present an efficient scheme to extract an optimal sub-stream from the whole SVC video bitstream under given bitrate constraint. A linear error model is used to accurately estimate the distortion caused by discarding any combination of data packets, and a greedy-like algorithm is designed to assign each data packet a priority value for Rate-Distortion optimized extraction. Experimental results show that the PID-based quality control algorithm brings 8.6\% higher video quality with 24.8\% lower quality fluctuation comparing with the baseline algorithm, and the bitstream extraction method achieves a PSNR gain of up to 0.5dB with no computational complexity increment. Altogether, the system produces a higher and smoother PSNR curve representing the final video quality perceived by the user. The proposed system is also implemented in online video website www.7dlive.com and proves to be practically effective.
\end{abstract}
...
After combining the two algorithms together, the proposed adaptive streaming system is able to deliver a higher and smoother video quality for the user. We compare the proposed system's performance with DSS that supports SVC but has its original quality control method and JSVM Basic bitstream extraction. The result for the same test stream as used in subsection \ref{subsec:exp-control} is present in Fig. \ref{fig:fluctuation-psnr}. It shows PSNR change perceived by the user when available bandwidth fluctuates around 800 kbps. It can be seen that, the proposed system has effectively avoided unnecessary quality drop (note the curve around time 50s and 350s) and been able to keep a relatively higher PSNR value (note the curve between time 450s and 600s).
