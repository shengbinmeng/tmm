...
\title{Scalable Video Streaming with PID-Based Quality Control and Optimized Bitstream Extraction}
...
\begin{abstract}
In order to meet the challenge brought by bandwidth fluctuation of the user's network, modern video streaming systems tend to be scalable, i.e., of flexible bitrate and adaptive video quality. This paper gives solutions for two key problems within such a scalable video streaming system. We first propose a quality control algorithm, which allows the video streaming system to adjust its video quality along with the real bandwidth change. The algorithm is based on the Proportional-Integral-Derivative (PID) control method and can deliver best video quality with least quality fluctuation. Then for streaming systems that adopt Scalable Video Coding (SVC), we present an efficient solution for the bitstream extraction problem, which aims to select an optimal sub-stream from the whole bitstream under given bitrate constraint. This solution features a linear error model, which is utilized to estimate the distortion caused by discarding any combination of data packets, and a greedy algorithm scheme, which assigns each data packet a priority value to use for the R-D optimized extraction. Experimental results show that our quality control algorithm can drastically reduce the quality fluctuation while still keeping a high quality level, and the bitstream extraction method can achieve a significant PSNR gain compared to the extractors of JSVM, without computational complexity increment. The quality control algorithm has also been implemented in online video website www.7dlive.com and turns out to be practically effective in applications.
\end{abstract}
...
% 
\IEEEPARstart{V}{ideo} streaming applications are becoming more and more popular on the Internet. Examples of the most common usage scenarios include VOD (Video On Demand), live broadcasting, video-based online education, and so on. In these applications, video data is constantly delivered to different clients over various transmission channels, and the client media player can begin playing the video before the entire file has been transmitted. Among other features, video streaming makes it easy to watch video anywhere and anytime, and is an increasingly important way for people to consume video content.

One of the biggest challenge for video streaming is the variety of users' network environment. The users may use various types of network (Ethernet, Wi-Fi, Cellular, etc) and may also switch to a different one on going (e.g., from Wi-Fi to cellular network). Even in the same network, a user's available bandwidth may change from time to time, influenced by the overall traffic. During such an environment, bandwidth fluctuation is inevitable. If the video is streamed at a constant bitrate, it would be difficult or impossible for the client to keep a non-paused smooth playing. Therefore, to achieve better watching experience for users with various network environments, we should be able to adjust the video's bitrate or quality according to the bandwidth change. In other words, the video streaming system needs to be scalable.

There are two options to enable scalable video streaming. The first option is called simulcast, where several independent bitstreams are stored at the server side, and the client selects one of them according to its current bandwidth. Dynamic Adaptive Streaming over HTTP (DASH) \cite{DASH}, which is standardized by MPEG, mostly uses this option. Simulcast systems like DASH can provide some scalability and is relatively easy to deploy. However, the scalability it provides is limited to the number of streams, and the processing and storage of multiple streams significantly increase time and space cost. On the contrary, the second option, which is to adopt Scalable Video Coding (SVC) \cite{SVC}, does not have above shortcomings. Compared to simulcast, SVC provides seamless scalability and higher coding efficiency, thus is a better choice to build a scalable video streaming system. While our work in this paper can be partly adapted to the simulcast system, we will mainly focus on the second option, i.e., SVC streaming system.

The generally-known term SVC usually refers to the scalable extension of the H.264/AVC video coding standard \cite{SVCOverview}, which has provided an effective way to deal with the new challenges of modern video applications. SVC allows efficient scalability in terms of video frame rate, spatial resolution, and picture fidelity, with only one single bitstream. The SVC bitstream is composed of Network Abstract Layer Units (NALUs), which are separated into multiple layers: one base layer for basic video quality and several enhancement layers for video quality improvement. By discarding NALUs of the enhancement layers, the bitstream can be extracted as required, providing the most appropriate data rate and video service according to the client capability and network conditions.

In SVC streaming system, there are two important issues that need to be addressed. First is quality control, and the second is bitstream extraction. Quality control means controlling the video quality (as well as the bitrate) to follow the change of bandwidth. Once the bandwidth has changed, the quality control algorithm should determine a bitrate suitable for the current bandwidth. Given this bitrate, how to extract from the whole SVC stream to form a optimal sub-stream becomes the bitstream extraction problem. Optimized bitstream extraction aims to select a subset of data which represents video with best quality, or minimal distortion, under the constrain of given bitrate. It should be noted that, bitstream extraction is not only useful in scalable video streaming, but also in other SVC application scenarios where part of data needs to be obtained or kept, e.g., the archiving of surveillance video.

In this paper, we propose novel solutions for these two issues and construct an efficient scalable video streaming system. Apple Darwin Streaming Server (DSS) \cite{DSS}, which has been widely used to collect, cache, schedule and transmit multimedia contents in practice, is chosen as the basis of the system. We modified the original DSS project and added SVC support to it, for the purpose of both research and application. A new quality control algorithm based on the Proportional-Integral-Derivative (PID) control concept is proposed to replace the native one in DSS, and achieves higher average quality level with less quality fluctuation. In this algorithm, we can monitor and even predict the relationship between the network bandwidth and the current video data rate, so that quality adjustment can be made at any time and to any extent, accurately and smoothly. For the bitstream extraction problem, we propose a linear error model to directly estimate pixel value errors before MSE or PSNR is calculated. The linear error model exploits the linear feature of H.264/AVC coding process and achieves good performance in distortion estimation. Using this model, the error caused by the removal of each enhancement packet can be obtained independently. This enables us to estimate the distortion caused by discarding any combination of packets. Based on that, we design a greedy algorithm to calculate the priority of each quality enhancement packet according to its R-D impact. The priority value of packets can be stored in the bitstream and used for optimized extraction. The proposed algorithms perform well in experiments and the quality control algorithm is also used in practical streaming systems and proved quite effective.
...
In this section, we analyze the two most concerned problems in SVC streaming, i.e., quality control and bitstream extraction, and introduce some existing related works on them.
...
\caption{Push Time and Play Time \label{fig:intervals}}
\end{figure}
...
\subsection{Error Vector Acquisition}
 ...
 \caption{SVC GOP structure \label{fig:GOP_Structure}}
 \end{figure}
