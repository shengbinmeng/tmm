...
\thanks{Manuscript received Febrary 15, 2015.}
...
\begin{abstract}
In order to give the user best quality of service under bandwidth fluctuation, video streaming systems should be able to adjust the video quality according to available bandwidth. In this paper we propose novel models and algorithms to enable such an adaptive video streaming system which adopts the Scalable Video Coding (SVC) technology. First, we utilize the Proportional-Integral-Derivative (PID) method in quality control to determine a suitable transmission bitrate according to actual bandwidth condition. By considering bandwidth information of the past, the current and even the future, the PID-based quality control algorithm is able to deliver highest video quality with least quality fluctuation. Second, we present an efficient bitstream extraction scheme to select an optimal sub-stream from the whole SVC video bitstream under given bitrate constraint. A linear error model is used to accurately estimate the distortion caused by discarding any combination of data packets, and a greedy-like algorithm is designed to assign each data packet a priority value for Rate-Distortion optimized extraction. Experimental results show that comparing with the existing solutions, the PID-based quality control algorithm brings 8.6\% higher video quality with 24.8\% lower quality fluctuation, and the bitstream extraction method achieves a PSNR gain of up to 0.5dB with no computational complexity increment. The proposed algorithms are implemented in online video website www.7dlive.com and prove to be practically effective.
\end{abstract}
...
\subsection{Experiments for combined system}

After combining the two algorithm together, we compared the proposed system's performance with the original DSS with JSVM Basic bitstream extraction. The result can be seen in 
Fig. \ref{fig:fluctuation-psnr}.

\begin{figure}[t]
\centering
\includegraphics[width = 0.9\linewidth]{Fluctuation-PSNR.jpg}
\caption{PSNR fluctuation with time \label{fig:fluctuation-psnr}}
\end{figure}

\begin{figure*}[t]
\centering
\includegraphics[width = 0.9\linewidth]{Fluctuation-PSNR-wide.jpg}
\caption{PSNR fluctuation with time \label{fig:fluctuation-psnr-wide}}
\end{figure*}
