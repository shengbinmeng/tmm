...
With relatively simple distortion estimation, Yang et al. \cite{Yang13} proposed a method based-on simulated annealing to determine priority values used in bitstream extraction. Ramanathan et al. \cite{Ramanathan12} proposed their concept and calculation method of the ``Quality Metadata'' to signal and use in bitstream extraction. In both methods, the accuracy of distortion estimation still affects the performance a lot.

In our previous work \cite{Zhang12}, we proposed a linear error model to directly estimate pixel value errors before MSE or PSNR is calculated, which achieves good performance in distortion estimation and bitstream extraction. However, this work assumes that the original video sequence is available for reference, which is not always true. How to handle the special characteristic of the no-reference case is lacking in the previous work and included in this paper.
