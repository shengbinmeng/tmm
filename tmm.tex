...
\title{Adaptive Video Streaming with PID-Based Quality Control and Optimized Bitstream Extraction}
...
\thanks{Manuscript received April 10, 2015.}
...
\begin{abstract}
To cope with the challenge brought by bandwidth fluctuation and provide the user with best quality of service, this paper proposes an adaptive video streaming system which adopts the Scalable Video Coding (SVC) technology and integrates novel models and algorithms. First, the Proportional-Integral-Derivative (PID) method is utilized to control the video quality adjustment according to actual bandwidth condition. By considering bandwidth information of the past, the current and even the future, the PID-based quality control algorithm determines a suitable bitrate for transmission. Then, under the constraint of that bitrate, an efficient scheme is used to extract an optimal sub-stream from the whole SVC video bitstream. In this scheme, a linear error model enables accurate estimation of the distortion caused by discarding any combination of data packets, and a greedy-like algorithm is designed to assign each data packet a priority value for Rate-Distortion optimized bitstream extraction. Experimental results show that by combining the above algorithms, PSNR average can be increased from 40.29 to 40.62, and PSNR variance can be reduced from 0.66 to 0.28, i.e., both higher video quality and lower quality fluctuation are achieved. The proposed system is also implemented in online video website www.7dlive.com and proves to be practically effective.
\end{abstract}
...
The advantage of simulcast systems is that they need little modification of existing infrastructure and therefore are easy to deploy. Most commercial video websites, YouTube \cite{YouTube} for instance, use this option to enable quality adaptation. However, the adaptive range and granularity that simulcast provides is limited to the number of streams (YouTube videos have 5 quality levels at most: 240p, 360p, 480p, 720p and 1080p), and the processing and storage of multiple video streams significantly increase time and space cost. SVC streaming systems, on the contrary, do not have above shortcomings. Since an SVC video bitstream can be extracted at the granularity of one NALU (a very small partition of data), it is able to provide seamless scalability and quality adaptation. Besides, performance analysis \cite{SVCPerformance} shows that SVC also has much higher coding efficiency than simulcast, despite some computation and bitrate overhead comparing with single layer stream. Therefore, we choose to adopt SVC in our adaptive video streaming system. While the work of this paper can be partly used in a simulcast system, it will mainly focus on the adaptive streaming system using SVC.

In the SVC video streaming system, one scalable video stream with enhancement data packets is produced according to actual requirement and stored at the server for streaming (how to configure and produce the stream is not in the scope of this paper; users are referred to \cite{JSVM}). During the streaming process, there are two key problems that need to be addressed well. The first is quality control, and the second is bitstream extraction. Quality control means controlling the video quality (or the video bitrate, from the transmission point of view) to follow the change of the bandwidth. Once the bandwidth has changed, the quality control algorithm should determine a bitrate suitable for the current bandwidth. Given this bitrate, how to extract data from the whole SVC stream to form an optimal sub-stream becomes the second problem, i.e., bitstream extraction problem. Optimized bitstream extraction aims to select a subset of NALUs (or packets) which represents video with best quality, or minimal distortion, under the constraint of the given bitrate. It should be noted that, bitstream extraction is not only useful in adaptive video streaming, but also in other SVC application scenarios where part of data needs to be obtained or kept, e.g., the archiving of surveillance video.

In this paper, an efficient adaptive video streaming system based on SVC is proposed with the above two problems better solved. Apple Darwin Streaming Server (DSS) \cite{DSS}, which is open-sourced and has been widely used to stream multimedia contents in practice, is chosen as the basis of the system. The original DSS project is modified by us and SVC support is added to it, for the purpose of both research and application. Then novel algorithms of quality control and bitstream extraction are designed and implemented to achieve significant improvement in terms of user-perceived video quality. The contributions of our work are threefold:
\begin{itemize}
\item A quality control algorithm based on the Proportional-Integral-Derivative (PID) control concept is designed to replace the native one in DSS, and achieves higher average video quality with less quality fluctuation.
\item A linear error model is proposed to accurately estimate the distortion caused by discarding any combination of packets from a SVC bitstream, based on which a greedy-like bitstream extraction method is proposed and achieves PSNR gain of up to 0.5dB.
\item The proposed novel solutions for quality control and bitstream extraction are combined together and result in a higher and smoother PSNR curve representing the final video quality perceived by the user.
\end{itemize}

The rest of this paper is organized as follows. In section \ref{sec:analysis}, we analyze the problems in detail and introduce some related works on them. Then in section \ref{sec:quality-control}, the proposed quality control algorithm is formulated and explained with term definitions and parameters selection. After that, we present the solution for the bitstream extraction problem in section \ref{sec:bit-extraction}, including the linear error model and the greed-like algorithm. Section \ref{sec:experiment} lists the experimental results and compares our proposed solution with others. Finally, in section \ref{sec:conclusion}, we conclude this paper's work and point out its promising applications and extensions in the future.
...
The two most concerned problems in SVC adaptive streaming, i.e., quality control and bitstream extraction, are illustrated in Fig. \ref{fig:problems}. In this section, we analyze the two problems in detail, and introduce some existing related works on them.

\begin{figure}[t]
\centering
\includegraphics[width = 0.9\linewidth]{Problems.jpg}
\caption{Two key problems in adaptive video streaming\label{fig:problems}}
\end{figure}
...
SVC bitstream extraction is substantially a combinatorial optimization problem. More specifically, it is in the form of ``0-1 Knapsack Problem" \cite{Knapsack}. In the ``0-1 Knapsack Problem", given a set of objects, each with a weight and a value, one decides which object to include in the knapsack so that the total weight is no more than a given limit and the total value is as large as possible. Similarly, in bitstream extraction problem, each NALU data packet can be treated as an item with certain weight (the packet's size) and value (its contribution to the video quality). The solving process is mainly to decide whether or not a packet should be included in the final extracted sub-stream, or in the term of ``0-1 Knapsack Problem", to choose which object to pack up.
...
The following of this paper will present solutions for the above problems in detail. We have published the related ideas in conference papers \cite{Meng14}\cite{Meng14-PID}, and in this paper we combine our previous works together and lay a solid foundation for a practical adaptive video streaming system.
...
After the quality control algorithm determines an appropriate bitrate and sets a quality level accordingly, the system will only send packets belonging to this new quality level (as described at the end of Algorithm \ref{algo:control}). For SVC videos, it means to extract a subset of packets from the whole bitstream. How to optimally choose this subset to achieve minimum distortion under the given bitrate is the key point of bitstream extraction problem. In this section, we propose a linear error mode and a greedy-like algorithm to solve this problem.
...
To verify the linear error model, we conduct an experiment where possible combinations of enhancement packets are removed from a video bitstream, and the distortion estimated by the linear error model are compared with the actual distortion caused by the removal. The video stream has 8-frame GOPs and three quality layers, the same as illustrated in Fig. \ref{fig:GOP_Structure}. Experimental result of the sequence {\em Foreman} is partly shown in Fig. \ref{fig:linear_verification}. The horizontal axis stands for sample point numbers of those possible combinations, and the vertical axis for MSE measurement. The solid line represents the actual MSE of a randomly extracted substream, and the dot line shows the estimated MSE using our Linear Error Model. A 0.6\% maximum relative error of MSE demonstrates the accuracy of the proposed model.
...
To implement our proposed adaptive streaming system requires the combination of the PID-based quality control algorithm and the optimized bitstream extraction method. The former determines the current bitrate to follow the bandwidth change, and the latter will ensure the sub-stream with the best possible quality is obtained and sent under that bitrate constraint. They work together to construct a system that delivers best quality of experience for the users. Choosing DSS as the implementation basis, we have integrated the proposed algorithms into it for the purpose of both research and application. In this section, we first present separate experiments to demonstrate the advantage of each proposed solution, and then give the result when they are combined together. Readers who are interested in the practical implementation can also visit the online video website http://www.7dlive.com for more information.
...
After combining the two algorithms together, the proposed adaptive streaming system is able to deliver a higher and smoother video quality for the user. We compare the proposed system's performance with DSS that supports SVC but has its original quality control method and JSVM Basic bitstream extraction. The result for the same test stream as used in subsection \ref{subsec:exp-control} is present in Fig. \ref{fig:fluctuation-psnr}. It shows PSNR change perceived by the user when available bandwidth fluctuates around 800 kbps. It can be seen that, the proposed system has effectively avoided unnecessary quality drop (note the curve around time 50s and 350s) and been able to keep a relatively higher PSNR value (note the curve between time 450s and 600s). The average and variance of PSNR delivered by the proposed system are 40.62 and 0.28, respectively, while the values of the baseline system are 40.29 and 0.66.

\begin{figure*}[t]
\centering
\includegraphics[width = 0.9\linewidth]{Fluctuation-PSNR-wide.jpg}
...
In this paper, we propose an adaptive video streaming system with PID-based quality control algorithm and optimized bitstream extraction. The PID-based quality control algorithm has a flexible mechanism to determine the suitable quality level so that it gives an accurate and swift response to the bandwidth change. Comparing with the original algorithm in DSS, this algorithm has reduced quality fluctuation significantly to improve audience satisfaction. The bitstream extraction method includes a linear error model to estimate the distortion caused by discarding any combination of packets and a greedy algorithm to assign priority to each data packet. The packets' priority values can then be stored in the bitstream and used for R-D optimized extraction. Experimental results show that the proposed method can achieve PSNR gain of up to 0.5dB compared to the JSVM QL extractor under the same bitrate constraint, also with the same computational complexity. The SVC streaming system integrated with these two solutions performs well in automatically adapting to the user's bandwidth fluctuation, and is able to give the user a better watching experience.

The PID-based quality control algorithm is only network-concerned thus should be easily integrated into other streaming systems, e.g., simulcast system like DASH. For the bitstream extraction, since the next generation video coding standard High Efficiency Video Coding (HEVC) \cite{HEVC} adopts the similar coding framework as its predecessor H.264/AVC, we expect to apply this paper's model and approach to the emerging HEVC standard and its scalable extension in our future work.
...
\bibitem{HEVC}
G.~J.~Sullivan, J.-R.~Ohm, W.-J.~Han, and T.~Wiegand, ``Overview of the High Efficiency Video Coding (HEVC) standard'' in {\em IEEE Trans. Circuits Syst. Video Technol.}, vol.~22, no.~12, pp.~1648â1667, Dec. 2012. 
